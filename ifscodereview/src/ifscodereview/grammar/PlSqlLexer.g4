...
// IFS rules
IFS_PROJECT_LAYER : 'layer' ~';' ('\r'? '\n')? -> channel(HIDDEN);

CONDITIONAL_IF_WITH_PARENS : '$IF' '(' .*? ')' '$THEN' -> channel(HIDDEN);
CONDITIONAL_IF_WITHOUT_PARENS : '$IF' .*? '$THEN' -> channel(HIDDEN);
CONDITIONAL_THEN : '$THEN' -> channel(HIDDEN);
CONDITIONAL_ELSE : '$ELSE' -> channel(HIDDEN);
CONDITIONAL_END : '$END' -> channel(HIDDEN);

ANNOTATION : '@' [A-Z]+ ('(' .*? ')')? -> channel(HIDDEN);
